...
function Raycast(line, x1, y1, x2, y2, d)
{
	...
	var ymin = 400; // used for occlusion
	for (var i = 1; i < r|0; i++)
	{
		x1 += dx;
		y1 += dy;
...
{
	load_binary_resource('/comanche/data/map' + index + '.palette', 3);
	load_binary_resource('/comanche/data/map' + index + '.height', 1);
	load_binary_resource('/comanche/data/map' + index + '.color', 2);
}
